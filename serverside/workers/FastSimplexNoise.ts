...
export class FastSimplexNoise {
  ...
  constructor(options: Options = {}) {
    if (options.hasOwnProperty('amplitude')) {
      if (typeof options.amplitude !== 'number') throw new Error(`options.amplitude must be a number. Received ${options.amplitude}`);
      this.amplitude = options.amplitude;
    } else this.amplitude = 1.0;

    if (options.hasOwnProperty('frequency')) {
      if (typeof options.frequency !== 'number') throw new Error(`options.frequency must be a number. Received ${options.frequency}`);
    ...
    if (options.hasOwnProperty('octaves')) {
      if (typeof options.octaves !== 'number' ||
        ...
        throw new Error(`options.octaves must be an integer. Received ${options.octaves}`);
      }
      this.octaves = options.octaves;
    } else this.octaves = 1;

    if (options.hasOwnProperty('persistence')) {
      if (typeof options.persistence !== 'number') throw new Error(`options.persistence must be a number. Received ${options.persistence}`);
      this.persistence = options.persistence;
    } else this.persistence = 0.5;

    if (options.hasOwnProperty('random')) {
      if (typeof options.random !== 'function') throw new Error(`options.random must be a function. Received ${typeof options.random}`);
      this.random = options.random;
    } else this.random = Math.random;

    let min: number;
    if (options.hasOwnProperty('min')) {
      if (typeof options.min !== 'number') throw new Error(`options.min must be a number. Received ${options.min}`);
      min = options.min;
    } else min = -1;

    let max: number;
    if (options.hasOwnProperty('max')) {
      if (typeof options.max !== 'number') throw new Error(`options.max must be a number. Received ${options.max}`);
      max = options.max;
    } else max = 1;

    if (min >= max) throw new Error(`options.min (${min}) must be less than options.max (${max}). Received ${min} >= ${max}`);
  ...
  cylindrical(circumference: number, coords: number[]): number {
    switch (coords.length) {
      ...
      default:
        throw new Error('coords must be 2 or 3 dimensions');
    }
  }
  ...
  raw(coords: number[]): number {
    switch (coords.length) {
      ...
      default:
        throw new Error(`Coords must be 2, 3, or 4 dimensions. Received ${coords.length}`);
    }
  }
  ...
  scaled(coords: number[]): number {
    switch (coords.length) {
      ...
      default:
        throw new Error(`Coordinate count must be 2, 3, or 4. Received ${coords.length}`);
    }
  }
  ...
  spherical(circumference: number, coords: number[]): number {
    switch (coords.length) {
      ...
      default:
        throw new Error(`Coordinate count must be 2 or 3. Received ${coords.length}`);
    }
  }
